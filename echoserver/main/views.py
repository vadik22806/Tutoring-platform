...
def tutors_list(request):
    ...
    # Попытка сортировки — если драйвер БД (djongo) не поддерживает ORDER BY,
    # падаем назад на несортированный список.
    from django.db import DatabaseError
    tutors_qs = User.objects.filter(role='tutor').order_by('first_name')
    try:
        tutors = list(tutors_qs)
...
def students_list(request):
    ...
    from django.db import DatabaseError
    students_qs = User.objects.filter(role='student').order_by('first_name')
    try:
